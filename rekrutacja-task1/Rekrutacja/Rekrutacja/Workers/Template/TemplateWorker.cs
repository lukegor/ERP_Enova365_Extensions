...
namespace Rekrutacja.Workers.Template
{
    public class TemplateWorker
    {
        ...
        private void UpdateEmployee(Session sesja, Pracownik emp)
        {
	        ...
	        pracownik.Features["Wynik"] = Calculate();
		}

        private double Calculate()
        {
			switch (Parametry.Operation)
			{
				case '+':
				...
				case '*':
					return Parametry.Value1 * Parametry.Value2;
				case '/':
					if (Parametry.Value2 == 0)
						throw new DivideByZeroException("Nie można dzielić przez 0");
					return Parametry.Value1 / Parametry.Value2;
				default:
					throw new InvalidOperationException($"Nieprawidłowy operator: {Parametry.Operation}");
			}
		}
    }
}